...
class AppComponent extends Component {
  ...
  removeRow = () => {
    this.setState((prevState) => {
      ...
      if (gridData.length > 1 || prevState.rows > 1) {
        //check if grid has more than one row or if the prevState has more than 1
  ...
  removeColumn = () => {
    this.setState((prevState) => {
      //this makes state into prevState
      const { gridData } = prevState;
      if (gridData[0].length > 1) {
        //checks if the grid has more than one column
